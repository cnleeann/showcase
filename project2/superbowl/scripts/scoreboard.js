var scale = 5;
  ...
  // LABELS
  scoreSVG.append("text").attr("x", (scoreWidth/9)).attr("y", (scoreHeight/9)+1.1*scale).style("fill", "white").text("BAL").style("text-anchor", "middle");
  scoreSVG.append("text").attr("x", scoreWidth*8/9).attr("y", (scoreHeight/9)+1.1*scale).style("fill", "white").text("SF").style("text-anchor", "middle");
  scoreSVG.append("text").attr("x", scoreWidth/9).attr("y", (scoreHeight*2/3)).style("fill", "white").text("DOWN").style("text-anchor", "middle");
  scoreSVG.append("text").attr("x", (scoreWidth/3)+1.2*scale).attr("y", (scoreHeight*2/3)).style("fill", "white").text("TO GO").style("text-anchor", "middle");
  scoreSVG.append("text").attr("x", (scoreWidth*2/3)-2*scale).attr("y", (scoreHeight*2/3)).style("fill", "white").text("BALL ON").style("text-anchor", "middle");
  scoreSVG.append("text").attr("x", scoreWidth*8/9).attr("y", (scoreHeight*2/3)).style("fill", "white").text("QTR.").style("text-anchor", "middle");
  ...
  // BALL POSSESSION
  var hPoss = scoreSVG.append("rect")
                              .attr("x", (scoreWidth/4)+0.7*scale)
                              .attr("y", (scoreHeight/3)-1.2*scale)
                              .attr("height", (1.5*scale))
                              .attr("width", 3*scale)
                              .attr("id", "homePossession");
  var gPoss = scoreSVG.append("rect")
                              .attr("x", (scoreWidth*3/4)-4*scale)
                              .attr("y", (scoreHeight/3)-1.2*scale)
                              .attr("height", (1.5*scale))
                              .attr("width", 3*scale)
                              .attr("id", "guestPossession");
  ...
  var updateScoreboard= function(prevPlay, currentPlay) {
    ...
    if (prevPlay["off"] === "BAL") {
      hPoss.style("fill", "white");
      gPoss.style("fill", "black");
    } else {
      hPoss.style("fill", "black");
      gPoss.style("fill", "white");
    }
